...
function test_create_cluster() {
  ...
  KWOK_CONTROLLER_PORT=10247 KWOK_KUBE_VERSION="${release}" kwokctl -v=-4 create cluster --name "${name}" --timeout 10m --wait 10m --quiet-pull --config="${DIR}/cluster-port-forward.yaml"
  if [[ $? -ne 0 ]]; then
...
function test_cluster_port_forward() {
  local name="${1}"
  local pid
  local result
...
function test_apply_node_and_deployment() {
  ...
  kwokctl --name "${name}" kubectl wait deploy/fake-pod --for condition=available --timeout 10s || exit 1
}
...
function main() {
  ...
  for release in "${RELEASES[@]}"; do
    ...
    test_cluster_port_forward "${name}" "8888" || failed+=("cluster_port_forward_${name}_default_forward")
    test_cluster_port_forward "${name}" "8001" || failed+=("cluster_port_forward_${name}_target_forward")
    test_cluster_port_forward "${name}" "8002" || failed+=("cluster_port_forward_${name}_command_forward")
